...
import { MessageCircle, X, Send, Trash2, Bot, User, HelpCircle, MessageSquare, Brain } from 'lucide-react'
  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="fixed bottom-6 right-6 w-14 h-14 rounded-2xl bg-navy-700 text-white shadow-xl flex items-center justify-center z-50"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-80 sm:w-96 h-[520px] bg-white dark:bg-navy-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-navy-700 flex flex-col z-50 overflow-hidden"
          >
            {/* Header */}
            <div className={`flex items-center gap-3 p-4 border-b border-slate-100 dark:border-navy-700 rounded-t-2xl text-white transition-colors ${socratic ? 'bg-teal-700' : 'bg-navy-700'}`}>
              ...
              <div className="flex-1">
                ...
                <p className={`text-xs ${socratic ? 'text-teal-200' : 'text-navy-200'}`}>
            ...
            {/* Mode badge */}
            {socratic && (
              <div className="px-4 py-2 bg-teal-50 dark:bg-teal-950/40 text-xs text-teal-700 dark:text-teal-300 font-medium border-b border-teal-100 dark:border-teal-900 flex items-center gap-1.5">
                <Brain size={13} /> Socratic mode: I'll ask guiding questions to help you discover answers yourself
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {messages.map((m, i) => (
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${m.role === 'user' ? 'bg-navy-500' : socratic ? 'bg-teal-700' : 'bg-navy-600'}`}>
              ...
              {loading && (
                <div className="flex gap-2">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center ${socratic ? 'bg-teal-700' : 'bg-navy-600'}`}>
                    {socratic ? <HelpCircle size={14} className="text-white" /> : <Bot size={14} className="text-white" />}
                  </div>
                  <div className="bg-slate-100 dark:bg-navy-700 rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1">
                    {[0, 0.15, 0.3].map((d, i) => (
            ...
            <div className="p-3 border-t border-slate-100 dark:border-navy-700">
              <div className="flex gap-2">
                ...
                <button onClick={sendMessage} disabled={!input.trim() || loading}
                  className={`p-2.5 rounded-xl shrink-0 text-white transition-colors ${socratic ? 'bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300' : 'btn-primary'}`}>
                  <Send size={16} />
                </button>
              </div>
            </div>
